...
@Component({
  ...
  styleUrl: './personal-lista.component.scss'
})
export class PersonalListaComponent implements OnInit {
  origen: string = "Registro Personal"
  ...
  filtrar() {
    this._personalEmpresaService.getAll().subscribe({
      next: (data: PersonalEmpresaResponse[]) => {
        this.loading = false;
        this.personalEmpresa = data;
        console.log('traje: ', this.personalEmpresa);
  ...
  async eliminar(personal: PersonalEmpresaResponse) {
    ...
    if (result) {
      this._personalEmpresaService.delete(personal.id).subscribe({
        next: () => {
